...
const SocialIcons = {
  ...
  const SocialLink = ({ icon, url }) => {
    ...
    return (
      <li>
        <a href={url} title={icon} className="flex items-center justify-center text-black transition-all duration-200 rounded-full w-7 h-7 ">
          <IconComponent />
        </a>
      </li>
    );
...
const FooterThree = ({logo,socialMediaLinks,navigationLinks,copyrightText}) => {


  return (
    <footer className="py-12 bg-white sm:py-16 lg:py-20">
      <div className="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <img className="w-auto h-8 mx-auto" src={logo.src} alt={logo.alt} />

        {/* Navigation Links */}
        <ul className="flex flex-wrap items-center justify-center gap-8 sm:gap-0 md:space-x-16 mt-14">
          {navigationLinks.map((link, index) => (
            <li key={index}>
              <a href={link.url} title={link.title} className="inline-flex text-lg font-medium text-gray-900 transition-all duration-200 transform font-pj hover:-translate-y-1 hover:text-gray-600">
                {link.title}
              </a>
            </li>
          ))}
        ...
        {/* Copyright Text */}
        <p className="text-base font-normal text-center text-gray-600 mt-7 font-pj">{copyrightText}</p>
      </div>
    </footer>
  );
};
