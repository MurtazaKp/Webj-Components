  ...
  return (
    <section className="py-12 bg-white sm:py-16 lg:py-20 xl:py-24">
      <div className="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
        ...
        <div className="grid max-w-2xl grid-cols-1 gap-5 mx-auto mt-12 sm:gap-6 sm:grid-cols-2 sm:mt-16 lg:mt-20 sm:max-w-none">
          {blogs.map((post) => (
            <div key={post.id} className="flex flex-col p-6 transition-all duration-200 bg-white border border-gray-200 sm:flex-row sm:items-stretch group rounded-2xl hover:shadow-lg">
              <a href="#" title="" className="flex overflow-hidden rounded-xl shrink-0">
                <img
                  className="object-cover w-full transition-all duration-200 transform h-44 group-hover:scale-110 group-hover:rotate-3"
                  src={post.image} alt={post.title} />
              </a>

              <div className="flex flex-col flex-1 mt-6 sm:mt-0 sm:ml-8">
                ...
                <div className="mt-6 lg:mt-auto">
                  <a href="#" title="" className="flex items-center text-base font-medium text-gray-600 hover:text-gray-900">
                    <img className="object-cover mr-2.5 rounded-full w-7 h-7 shrink-0" src={post.author.avatar} alt={post.author.name} />
                    {post.author.name}
                  </a>
                </div>
              </div>
